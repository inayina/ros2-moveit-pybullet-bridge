#!/usr/bin/env bash
# Verify NFR-S safety criteria: e-stop, soft limit, watchdog, degraded mode, recovery.
set -eo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
OUTPUT="${1:-${ROOT}/docs/samples/safety-nfr-metrics.json}"
...
echo "==> Launch portfolio_demo for NFR-S (DIRECT, dual source, camera off)"
setsid ros2 launch pybullet_bridge portfolio_demo.launch.py \
  ...
  >/tmp/nfr_s_verify_portfolio.log 2>&1 &
PORT_PID=$!
...
echo "==> Measure NFR-S safety"
if ! python3 "${SCRIPT_DIR}/check_safety_nfr.py" --output "${OUTPUT}"; then
  echo "==> portfolio log tail"
  tail -120 /tmp/nfr_s_verify_portfolio.log || true
  exit 1
fi

cleanup
trap - EXIT
echo "[PASS] NFR-S verification complete: ${OUTPUT}"
